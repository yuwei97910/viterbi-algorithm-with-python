...
def viterbi_3(train, test):
    ...
    for tag, word_dict in tag_word_dict.items():
        ...
        emission_dict[tag] = generate_emission_prob(tag, word_dict, smoothing_para)
    ...
    for i, word in enumerate(sentence):
        ...
        if i == 0:
            ...
            for tag in emission_dict.keys():
                ...
                p_e = get_emission_prob(word, tag, emission_dict)
        ...
        for tag_b in emission_dict.keys():
            ...
            for tag_a in v_dict_i.keys():
                ...
                p_e = get_emission_prob(next_word, tag_b, emission_dict)
...
def generate_emission_prob(tag, word_dict, smoothing_para):
    emission_tag_dict = {}

    common_pattern_dict = {"ing":0, "ly":0, "ment":0, "ed":0, "on":0, "able":0, "ess":0, "ous":0, "ive":0, "est":0, "s":0}
    total_pattern = len(common_pattern_dict.keys())

    v = len(list(word_dict.keys())) + total_pattern
    # v = len(list(word_dict.keys()))
    n = sum(list(word_dict.values()))
    emission_tag_dict = {}
    for word, cnt in word_dict.items():
        emission_tag_dict[word] = np.log((cnt+smoothing_para)/(n+smoothing_para * (v+1)))
        for pattern in common_pattern_dict.keys():
            if word.endswith(pattern):
                common_pattern_dict[pattern] += 1

    pattern_sum = 0
    for pattern, cnt in common_pattern_dict.items():
        emission_tag_dict[f'X-{pattern}'] = np.log((smoothing_para)*(cnt/v)/(n+smoothing_para*(v+1)))
        pattern_sum += cnt
    emission_tag_dict['UNKNOWN'] = np.log(smoothing_para*((v-pattern_sum)/v)/(n+smoothing_para*(v+1)))

    return emission_tag_dict

def get_emission_prob(word:str, tag:str, emission_dict:dict):
    word_dict = emission_dict[tag] # key: word, value: emission probability with tag
    if word in word_dict.keys():
        return word_dict[word]

    common_pattern = {"ing", "ly", "ment", "ed", "on", "able", "ess", "est", "ous", "ive", "s"}
    pattern = None
    for pattern in common_pattern:
        if word.endswith(pattern):
            # print(word, pattern, word_dict[f'X-{pattern}'])
            return word_dict[f'X-{pattern}']

    return word_dict['UNKNOWN']
